...
void huffman_encode(FILE *fpt,unsigned int file_size)
{
	...
	int ret = 0,i = 0,j = 0;
	unsigned int encoded_data_size=0,header_size = 0;
	...
	freq_count = (int *)calloc(256,sizeof(unsigned int));/*Allocating a counter for each possible byte (0-255)*/
	if(!freq_count)
	{
	...
	/*open file to store encoded data*/
	strcpy(encoded_file,"Huffman_ENCODED.dump");
	...
	ret = fwrite(encoded_data,1,header_size,encode_fpt);
	if(ret != header_size)
	{
	...
	memset(encoded_data,0,header_size);

	/*Encode the symbols in the file with its corresponding bit sequences*/
	EncodeData(encoded_data,original_data,file_size,
		symbol_list_head,node_pointers,
		&encoded_data_size,&total_bit_count);

	/*Write totalbits count to header in the file*/
	ret = fwrite((void*)&total_bit_count,sizeof(total_bit_count),1,encode_fpt);
	if(ret != 1)
	{
		printf("fwrite failed with %d\n",ret);
	}
	...
	printf("\nHuffman encoded file stored as %s with file size %d bytes\n\n",encoded_file,encoded_data_size+header_size+4);
...
void huffman_decode(FILE *fpt,unsigned int file_size)
{
	...
	unsigned char *original_data = NULL,*decoded_data = NULL;	
	node_data *head = NULL,*tail = NULL,*curr = NULL;
	node_data *symbol_list_head = NULL,*symbol_list_tail = NULL;
	unsigned int decoded_data_size=0, bits2decode = 0,byteindex = 0,uncompress_size = 0;
	int ret = 0,i = 0;
	...
	printf("\Huffman Decoded data saved as %s with file size %d bytes\n",decoded_file,decoded_data_size);
...
void CreateBinaryTree(node_data **head,node_data **tail)
{
	...
	while(curr)
	{
		...
		new_node = create_node(head,*tail,NULL,next->freq+curr->freq,curr,next,0);
...
void TraverseBinaryTree(node_data *symbol_list_head,unsigned int rootfreq)
{
	...
	/*Allocate memeory for the pattern*/
